...
function qwen3NoThinking(url: URL, init?: RequestInit) {
  if (init?.body) {
    try {
      const body = JSON.parse(init.body as string);
      if (typeof body.model === "string" && body.model.startsWith("qwen3")) {
...
const openai = createOpenAI({
  ...
  fetch: async (url, init) => fetch(qwen3NoThinking(url as URL, init)),
});
